...
class CombinedInterface(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.vector_scale_spinbox.setRange(0.001, 10.0)
        self.vector_scale_spinbox.setValue(0.1)
        self.vector_scale_spinbox.setSingleStep(0.01)
        self.vector_scale_spinbox.setDecimals(3)
        self.vector_scale_spinbox.valueChanged.connect(self.update_mesh)
    ...
    def add_magnetization_vectors(self, axes_angles):
        ...
        try:
            ...
            # Normalize vectors and calculate colors based on z-component
            norms = np.linalg.norm(rotated_vectors, axis=1, keepdims=True)
            normalized_vectors = rotated_vectors / np.maximum(norms, 1e-10)
            
            # Create color array based on z-component
            z_components = normalized_vectors[:, 2]
            colors = np.zeros((len(z_components), 3))
            
            # Red for up (mz > 0.5)
            up_mask = z_components > 0.5
            colors[up_mask] = [1.0, 0.0, 0.0]
            
            # Blue for down (mz < -0.5)
            down_mask = z_components < -0.5
            colors[down_mask] = [0.0, 0.0, 1.0]
            
            # Yellow for in-plane
            inplane_mask = ~(up_mask | down_mask)
            colors[inplane_mask] = [1.0, 1.0, 0.0]
            ...
            vector_poly['colors'] = colors
            
            # Create arrows using glyphs (much faster than individual arrows)
            ...
            # Add to plotter with RGB colors
